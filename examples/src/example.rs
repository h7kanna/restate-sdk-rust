use anyhow::Error;
use restate::{
    async_recursion, empty, endpoint::endpoint, full, http2_handler, BodyExt, BoxBody, Bytes, Context,
    Http2Connection, HttpIngress, Incoming, Method, Request, Response, StatusCode,
};
  ...
  "services": [
    {
      ...
      "handlers": [
        ...
        (&Method::POST, "/invoke/Greeter/greet") => {
            ...
            tokio::spawn(async move { http2_handler::handle(Service::service, http2conn).await });
        ...
        (&Method::POST, "/invoke/Greeter/greet2") => {
            ...
            tokio::spawn(async move { http2_handler::handle(SimpleService::greet, http2conn).await });
